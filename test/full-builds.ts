...
function checkForWarnings(stderr: string) {
    return stderr.toString().split('\n')
        .every(line =>
            line.indexOf('npm WARN') === 0 || // npm warnings
            line.indexOf('fs: re-evaluating native module sources is not supported. If you are using the graceful-fs module, please update it to a more recent version.') !== -1 || // confirmed gulp bug
            ...
            it('css', done => {
                ...
                generate(() => {
                    ...
                    exec('node_modules/.bin/gulp', (error, stdout, stderr) => {
                        ...
                        if (stderr) {
                            const onlyWarnings = checkForWarnings(stderr);
                            done(!onlyWarnings);
                        }
                    });
                });
